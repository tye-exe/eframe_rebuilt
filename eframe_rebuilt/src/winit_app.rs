use std::{cell::RefCell, marker::PhantomData, num::NonZeroU32, rc::Rc, sync::Arc, time::Instant};
...
use glutin::{
    ...
    prelude::{GlDisplay as _, NotCurrentGlContext as _, PossiblyCurrentGlContext as _},
...
use crate::{
    NativeOptions, UserEvent,
    epi::{App, AppCreator, CreationContext, HardwareAcceleration, Storage},
    epi_native::{
        EpiIntegration, apply_window_settings, create_storage, create_storage_with_file,
        load_window_settings, viewport_builder,
    },
    event_loop_context,
...
impl GlutinWindowContext {
    ...
    fn on_suspend(&mut self) -> Result<(), crate::Error> {
...
impl<'app> GlowWinitApp<'app> {
    ...
    ) -> Result<(GlutinWindowContext, egui_glow::Painter), crate::Error> {
        profiling::function_scope!();
        let window_settings = load_window_settings(storage);

        let winit_window_builder = viewport_builder(
    ...
    fn init_run_state(
        &mut self,
        event_loop: &ActiveEventLoop,
    ) -> Result<&mut GlowWinitRunning<'app>, crate::Error> {
        profiling::function_scope!();

        let storage = if let Some(file) = &self.native_options.persistence_path {
            create_storage_with_file(file)
        } else {
            create_storage(
                self.native_options
                    .viewport
